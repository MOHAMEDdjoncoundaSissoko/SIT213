...
public class Simulateur {
    ...
    public  Simulateur(String [] args) throws ArgumentsException {
        ...
        // création de la source :  message aléatoire ou message imposé
      	if (messageAleatoire) {
        ...
        // création du transmetteur (canal parfait, sans )
        transmetteurLogique = new TransmetteurParfait();
        destination = new DestinationFinale();
        
        // connexion de la chaine : source -> transmetteur -> destination
        source.connecter(transmetteurLogique);
        transmetteurLogique.connecter(destination);

        // connexion des sondes d'affichage si demandé
        if (affichage){
            source.connecter(new SondeLogique("Emmetteur",10));
